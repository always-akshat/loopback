...
var DEFAULT_TTL = 31556926; // 1 year in seconds
 ...
 * @property {String} realm The namespace the user belongs to. See [Partitioning users with
 *   realms](https://docs.strongloop.com/display/public/LB/Partitioning+users+with+realms) for
 *   details.
 * @property {Date} created The property is not used by LoopBack, you are free to use it for your
 *   own purposes.
 * @property {Date} lastUpdated The property is not used by LoopBack, you are free to use it for
 *   your own purposes.
 * @property {String} status The property is not used by LoopBack, you are free to use it for your
 *   own purposes.
 ...
 * @property {Number} settings.ttl Default time to live (in seconds) for the `AccessToken` created
 *   by `User.login() / user.createAccessToken()`. Default is `1209600` (2 weeks)
 * @property {Number} settings.maxTTL The max value a user can request a token to be alive / valid
 *   for. Default is `31556926` (1 year)
...
module.exports = function (User) {
	...
	User.prototype.createAccessToken = function (ttl, options, cb) {
		...
		this.accessTokens.create({
			ttl: ttl, model: userModel.definition.name
		}, cb);
		return cb.promise;
	};
	...
	User.login = function (credentials, include, fn) {
		...
		self.findOne({where: query}, function (err, user) {
			...
			function tokenHandler(err, token) {
				...
				// NOTE(akshat) custom code added for sending auth token as "auth" instead of "id"
				// when coming through Access_token table.
				token = token.toJSON();
				token.auth = token.id;
				delete token.id;
  ...
  *    });
   ...
   *    };
	...
	User._invalidateAccessTokensOfUsers = function (userIds, cb) {
		console.log("calling invalifation function");
